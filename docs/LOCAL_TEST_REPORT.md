...
最后更新：2026-07-13
测试机器：本机 macOS 开发环境  
功能验证基线：2026-07-13 `main` 维护基线

## 结论

Cliploom macOS 版本当前可作为日常自用版本。核心功能已经验证，后续项目默认进入
维护阶段：优先修复 Bug、更新文档和优化安装体验，不再主动加入大功能。
...
当前验证范围以 `PasteBoxTests` 为准，覆盖核心模型、服务、截图、OCR、扫码、翻译、
取色、面板几何和快捷键逻辑。`PasteBoxUITests` 未作为本轮发布阻断项。

## GitHub 状态说明

- `main` 分支代表最新源码。
- Releases 页面代表已打包安装包。
- 当前最新安装包仍为 Releases 页面中的最新 DMG，可能落后于 `main` 最新源码。
